...
@Slf4j
@Component
public class UserImportStrategy implements ImportStrategy<UserImportStrategy.ImportParams> {
    ...
    @Override
    //@change: 【已处理】  加入 or参数
    public IOutResult operation(IOutResult or, UserImportStrategy.ImportParams importParams) {
        String model = importParams.getModel();
        String userId = importParams.getUserId();
        if(importLimitCheck.countByModelAndUserId(model,userId) == importParams.getMaxUserTaskNum()){ //xxx
    ...
    public static class ImportParams{
        ...
        private int maxUserTaskNum; //同一模块最多允许几个task
